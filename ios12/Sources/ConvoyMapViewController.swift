...
final class ConvoyMapViewController: UIViewController, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItems = [
            UIBarButtonItem(title: "定位", style: .plain, target: self, action: #selector(recenter)),
            UIBarButtonItem(title: Identity.ghost ? "🕶" : "隐身", style: .plain, target: self, action: #selector(toggleGhost)),
        ]
    ...
    @objc private func toggleGhost() {
        Identity.ghost.toggle()
        navigationItem.rightBarButtonItems?.last?.title = Identity.ghost ? "🕶" : "隐身"
        renderHealth()
    }
    ...
    private func renderHealth() {
        if Identity.ghost {
            ...
            healthBanner.text = "🕶 隐身中：同伴看不到你的位置（仍能看到同伴）"
        } else if !MeshBus.shared.btAvailable {
            healthBanner.isHidden = false
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // 节流广播自身位置（每 3 秒），供同行者在地图上看到我；隐身时不广播（G-DR-3）
        let now = Date().timeIntervalSince1970
        if !Identity.ghost, now - lastBroadcast >= 3 {
            lastBroadcast = now
