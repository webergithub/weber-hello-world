...
public class BleMeshPlugin: CAPPlugin, CAPBridgedPlugin,
    ...
    private func onFrameReceived(_ frame: Data) {
        ...
        let id = bytes[0..<8].map { String(format: "%02x", $0) }.joined()
        let seq = (Int(bytes[8]) << 8) | Int(bytes[9])
        ...
        cleanupReasm()
        let r = reasm[id] ?? {
            let n = Reasm(total: total); reasm[id] = n; return n
        }()
        if r.chunks[seq] == nil {
            r.chunks[seq] = chunk
            r.received += 1
