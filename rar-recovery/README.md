...
如果你记得密码的**大致形状**（比如"一个词 + 4 位年份""大写开头 + 几位字母 + 几位数字"），
用掩码能把搜索空间缩到极小、精准命中。语法与 hashcat 完全一致（将来接 GPU 可直接复用）：
...
```bash
# 记得是 "love" + 4 位数字（如 love2008）
python3 recover.py x.rar --mask 'love?d?d?d?d'

# 记得是 大写开头 + 3 位小写 + 4 位数字（如 Wang2025）
python3 recover.py x.rar --mask '?u?l?l?l?d?d?d?d'

# 自定义：?1 只在这些字符里取
python3 recover.py x.rar --mask 'Pwd?1?1?1' --mask-custom1 'abc123!@'
```

网页/桌面版有专门的掩码输入框，会**实时显示这条掩码的组合数与预计耗时**。

> 💡 掩码空间别开太大：RAR/7z 每秒只能试几百次，`?u?l?d?d?d?d`（676 万）就要几小时，
> 这种规模请用下面的 GPU 方案（掩码字符串可直接给 hashcat 用）。
