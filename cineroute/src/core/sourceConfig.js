 ...
 * 进设置页，并且默认值是 `auto`——按已经配好了什么来挑：
 *
 *   配了 SERP 服务商 → api ；配了命令行工具 → cli ；都没配 → http
 *
 * `auto` **不会挑到 browser 或 ladder**，也不会去看这台机器上装没装浏览器。
 * 检索最终是在服务端跑的，服务器上一般没有图形环境更没有 Chrome；让默认
 * 路径取决于"能不能找到浏览器可执行文件"，等于让同一份配置在开发机和线上
 * 是两种行为。想用浏览器就到设置页明确选 browser 或 ladder。
 *
